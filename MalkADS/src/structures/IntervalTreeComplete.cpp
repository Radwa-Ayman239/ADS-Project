...
void RedBlackIntervalTree::updateMax(Node* node) {
    ...
    if (node->left != nullptr) leftMax = node->left->max;
    if (node->right != nullptr) rightMax = node->right->max;

    node->max = node->high;
    if (leftMax > node->max) node->max = leftMax;
...
// void RedBlackIntervalTree::listAvailableIntervalsHelper(Node* node, int StartLooking, int EndLooking, int &currenttime, std::vector<std::pair<int,int> > &availabletimes) {
//     if (node == nullptr) return;

//     if (node->left != nullptr && node->left->max >= StartLooking)
//         listAvailableIntervalsHelper(node->left, StartLooking, EndLooking, currenttime, availabletimes);

//     if (currenttime < node->low) {
//         int startFree = currenttime;
//         int endFree = node->low;
//         if (startFree < endFree) availabletimes.push_back(std::make_pair(startFree, endFree));
//     }
...
void RedBlackIntervalTree::remove(int low, int high) {
    ...
    if (yOriginalColor == BLACK) {
        ...
        else if (y->parent != nullptr) fixDelete(y->parent);
    }
