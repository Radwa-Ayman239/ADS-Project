...
public:
    ...
    py::list getRooms() {
        py::list roomsList;
        rooms.forEachRoom([&](const std::string& id) {
            roomsList.append(id);
        });
        return roomsList;
    }
    ...
    py::list getRoomBookings(const char* roomId) {
        ...
        rooms.getRoomBookings(roomId, [&](int start, int end, const std::string& username) {
            py::dict booking;
            booking["start"] = start;
            booking["end"] = end;
    ...
    py::list getBooks() {
        py::list booksList;
        books.forEachBook([&](const std::string& id) {
            booksList.append(id);
        });
        return booksList;
    }
    ...
    py::list searchBooks() {
        py::list booksList;
        books.forEachBookWithDetails([&](const std::string& id, const std::string& title, const std::string& author) {
            py::dict bookInfo;
            bookInfo["id"] = id;
    ...
    // Book a room
    py::dict bookRoom(const char* roomId, int start, int end, const char* username) {
        py::dict result;
        ...
        if (success) {
            ...
            result["message"] = "Conflict detected! Room is already booked or you have a conflicting booking.";
        }
    ...
    bool borrowLaptop(const std::string& laptopId, int start, int end, const std::string& username) {
        User* user = users.getUser(username);
    ...
    py::dict borrowAnyLaptop(int start, int end, const std::string& username) {
        ...
        std::string laptopId = laptops.borrowAnyLaptopDirect(user, start, end);
        if (!laptopId.empty()) {
            saveAll();
            result["success"] = true;
            result["message"] = "Laptop " + laptopId + " assigned successfully!";
            result["laptop_id"] = laptopId;
        } else {
            result["success"] = false;
            result["message"] = "No laptops available for this period or you have a conflict.";
        }
        return result;
    }
    
    // Borrow a book
    py::dict borrowBook(const char* bookId, int start, int end, const char* username) {
        py::dict result;
        ...
        if (success) {
            ...
            result["message"] = "Conflict detected! Book is not available or you've reached the limit of 3 concurrent books.";
        }
    ...
    py::list getUserBookings(const char* username) {
        ...
        // Room bookings
        rooms.forEachBooking([&](const std::string& resourceId, int start, int end, const std::string& bookedBy) {
        ...
        // Laptop bookings
        laptops.forEachBooking([&](const std::string& resourceId, int start, int end, const std::string& bookedBy) {
            if (bookedBy == username) {
                py::dict booking;
                booking["type"] = "laptop";
                booking["resource_id"] = resourceId;
        ...
        // Book bookings
        books.forEachBooking([&](const std::string& resourceId, int start, int end, const std::string& bookedBy) {
            if (bookedBy == username) {
                py::dict booking;
                booking["type"] = "book";
                booking["resource_id"] = resourceId;
    ...
    // Admin functions
    void addRoom(const char* roomId) {
        rooms.addRoomDirect(roomId);
    }
    
    void removeRoom(const char* roomId) {
        rooms.removeRoomDirect(roomId);
    }
    
    void addLaptop(const char* laptopId) {
        laptops.addLaptopDirect(laptopId);
    }
