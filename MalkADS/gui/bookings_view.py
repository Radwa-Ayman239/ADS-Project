...
from utils import seconds_to_datetime_string
...
class BookingsView(ctk.CTkToplevel):
    ...
    def __init__(self, parent, username, lib_system=None):
        ...
        # Close button
        StyledButton(
            main_frame, text="Close", variant="primary", width=150, command=self.destroy
    ...
    def display_bookings(self, parent):
        ...
        if rooms:
            self.display_category(
                parent, "🏛️ Room Bookings", rooms, COLORS["accent_blue"]
            )

        if laptops:
            self.display_category(
                parent, "💻 Laptop Bookings", laptops, COLORS["accent_purple"]
            )

        if books:
            self.display_category(
                parent, "📚 Book Bookings", books, COLORS["accent_green"]
            )
    ...
    def display_category(self, parent, title, bookings, color):
        ...
        header_frame.pack(
            fill="x", pady=(SPACING["md"], SPACING["sm"]), padx=SPACING["md"]
        )

        StyledLabel(header_frame, text=title, size="subheading", text_color=color).pack(
            anchor="w"
        )
        ...
        for booking in bookings:
            ...
            # Time range
            start_str = seconds_to_datetime_string(booking["start_time"])
            end_str = seconds_to_datetime_string(booking["end_time"])

            time_label = StyledLabel(
                booking_content,
                text=f"Time: {start_str} - {end_str}",
                size="body",
                text_color=COLORS["text_secondary"],
            )
