...
from widgets import StyledButton, StyledEntry, StyledLabel, Card, NotificationBanner, TimeSlotEntry
...
class BookingDialog(ctk.CTkToplevel):
    ...
    def __init__(self, parent, booking_type, username, lib_system=None):
        ...
        StyledLabel(
            form_frame,
            text=f"Select {booking_type.title()}:",
            size="body_large"
        ...
        if booking_type == "book":
            ...
            if booking_type == "laptop":
                ...
                self.resource_var = ctk.StringVar(value=resources[0] if resources else "")
    ...
    def on_resource_change(self, choice):
        ...
        if self.booking_type == "room" and hasattr(self, 'time_slots') and self.lib_system:
    ...
    def confirm_booking(self):
        ...
        if self.booking_type == "room":
            ...
            if start is None or end is None:
                self.show_notification("Please enter valid start and end times", "error")
                return
        ...
        try:
            if self.lib_system:
                if self.booking_type == "room":
                    result = self.lib_system.book_room(resource_id, start, end, self.username)
                elif self.booking_type == "laptop":
                    # Use auto-assignment for laptops
                    result = self.lib_system.borrow_any_laptop(start, end, self.username)
                elif self.booking_type == "book":
                    result = self.lib_system.borrow_book(resource_id, start, end, self.username)
