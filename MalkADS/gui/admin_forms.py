...
class ManagementDialog(ctk.CTkToplevel):
    ...
    def __init__(self, parent, action_type):
        ...
        header = ctk.CTkFrame(main_frame, fg_color=color, corner_radius=SIZES["button_corner"])
        ...
        # Notification area
        self.notification_frame = ctk.CTkFrame(form_frame, fg_color="transparent", height=50)
    ...
    def create_add_form(self, parent):
        ...
        if self.resource_type == "book":
            # Book ID
            StyledLabel(parent, text="Book ID:", size="body_large").pack(anchor="w", pady=(SPACING["md"], 5))
            self.id_entry = StyledEntry(parent, placeholder="e.g., B0001")
            self.id_entry.pack(fill="x", pady=(0, SPACING["md"]))
            
            # Title
            StyledLabel(parent, text="Title:", size="body_large").pack(anchor="w", pady=(0, 5))
            self.title_entry = StyledEntry(parent, placeholder="Book title")
            self.title_entry.pack(fill="x", pady=(0, SPACING["md"]))
            
            # Author
            StyledLabel(parent, text="Author:", size="body_large").pack(anchor="w", pady=(0, 5))
            ...
            StyledLabel(
                parent,
                text=f"{resource_name} ID:",
                size="body_large"
            ).pack(anchor="w", pady=(SPACING["md"], 5))
            
            placeholder = "e.g., L001" if self.resource_type == "laptop" else "e.g., R101"
            self.id_entry = StyledEntry(parent, placeholder=placeholder)
            self.id_entry.pack(fill="x", pady=(0, SPACING["md"]))
        
        # Info box
        info_box = ctk.CTkFrame(parent, fg_color=COLORS["bg_light"], corner_radius=SIZES["button_corner"])
    ...
    def create_remove_form(self, parent):
        ...
        # Warning box
        warning_box = ctk.CTkFrame(parent, fg_color=COLORS["warning"], corner_radius=SIZES["button_corner"])
    ...
    def show_notification(self, message, message_type="info"):
        ...
        icon = {"success": "✅", "error": "❌", "warning": "⚠️", "info": "ℹ️"}[message_type]
    ...
    def handle_add(self):
        ...
        if self.lib_system and LIBRARY_AVAILABLE:
            try:
                ...
                self.show_notification(f"{self.resource_type.title()} added successfully!", "success")
    ...
    def handle_remove(self):
        ...
        if self.lib_system and LIBRARY_AVAILABLE:
            try:
                ...
                self.show_notification(f"{self.resource_type.title()} removed successfully!", "success")
                self.after(1500, self.destroy)
            except Exception as e:
                self.show_notification(f"Error: {str(e)}", "error")
        else:
